...
public class AuthService {
     ...
     * <li>이메일 인증 완료 여부 확인 → 미완료면 EmailNotVerifiedException (Redis에
     * "verified:{email}" 키가 존재해야 함)</li>
     ...
     *
     * <p>
     * 힌트 — @Transactional: DB 쓰기 작업이 있으므로 이 메서드 위에 {@code @Transactional} 을
     * 추가해야 합니다. (클래스 레벨은 readOnly=true 로 되어 있으므로 쓰기 메서드에는 별도 선언 필요)
     */
    // ✅ 올바른 구현 — SignupRequest → User 변환 후 save
    public SignupResponse signup(SignupRequest request) {
        ...
        // 5. User 저장
        User savedUser = userRepository.save(user); // ✅ User 객체를 넘겨야 함
